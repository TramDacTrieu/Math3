...
public static class InterfaceHelper
{
    ...
    public static IList<T> FindObjectsResources<T>() where T : class
    {
        var resList = new List<T>();
        ...
        foreach (var curType in types)
        {
            var objects = Resources.FindObjectsOfTypeAll(curType);
            if (null == objects || objects.Length <= 0)
                continue;
    ...
    static IList<T> GetInterfaceComponents<T>(this Component component, bool firstOnly = false) where T : class
    {
        var types = _interfaceToComponentMapping[typeof(T)];
    ...
    static T GetInterfaceComponent<T>(this Component component) where T : class
    {
