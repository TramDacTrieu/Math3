...
public class InitScript : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        var adsCounter = FindObjectOfType<AdsCounter>();
        if (adsCounter == null)
        {
            var obj = new GameObject("AdsCounter");
            obj.AddComponent<AdsCounter>();

        }
        DateOfExit = PlayerPrefs.GetString("DateOfExit", "");
        Coins = PlayerPrefs.GetInt("Gems");
    ...
    public void CheckAdsEvents(GameState state)
    {
        ...
        foreach (AdEvents item in AdsCounter.THIS.adsEvents)
        {
    ...
    public void CheckRewardedAds()
    {
        ...
        if (currentReward == RewardedAdsType.GetGems)
        {
            ...
            reward.SetCoinsCount(rewardedGems);
            AddGems(rewardedGems);
            GameObject.Find("CanvasGlobal").transform.Find("CoinsShop").GetComponent<AnimationManager>().CloseMenu();
        }
    ...
    public delegate void BoostEvent(BoostType boostType);
    public static event BoostEvent OnBoostSpent;
    public static event BoostEvent OnBoostBought;
    public void BuyBoost(BoostType boostType, int price, int count)
    {
        PlayerPrefs.SetInt("" + boostType, count);
        PlayerPrefs.Save();
        OnBoostBought?.Invoke(boostType);
    ...
    public void OnLevelClicked(object sender, LevelReachedEventArgs args)
    {
        if (EventSystem.current.IsPointerOverGameObject(-1))
            return;
        if (!GameObject.Find("CanvasGlobal").transform.Find("MenuPlay").gameObject.activeSelf && !GameObject.Find("CanvasGlobal").transform.Find("CoinsShop").gameObject.activeSelf && !GameObject.Find("CanvasGlobal").transform.Find("LiveShop").gameObject.activeSelf)
        {
            PlayerPrefs.SetInt("OpenLevel", args.Number);
            PlayerPrefs.Save();
            LevelManager.THIS.MenuPlayEvent();
            LevelManager.THIS.LoadLevel(args.Number);
